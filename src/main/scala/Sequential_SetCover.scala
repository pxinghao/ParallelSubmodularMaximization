...
/**
 * Class implementing double greedy for set cover as a separable group sum objective function.
 ...
 */
class Sequential_SetCover(nElements: Int, nGroups: Int, graph: SparseGraph, lambda: Double)
  extends Sequential_SeparableGroupSums(nElements, nGroups, graph, lambda){
  protected def g(sumW: Double) = {
    math.min(1.0, sumW)
  }
