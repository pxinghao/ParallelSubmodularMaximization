...
/**
 * Class implementing double greedy for separable sums objective functions, of the form F(X) = \sum_l g(\sum_{i\in S_l\cap X} w_l(i)) - \lambda|X|.
 ...
 */
abstract class Sequential_SeparableGroupSums(val nElements: Int, val nGroups: Int, val graph: SparseGraph, val lambda: Double)
  extends Sequential_SubmodularDoubleGreedy(nElements){
  ...
  /**
   * \hat{\alpha} sketch: sum of weights of elements in A
   */
  val alphhat : AtomicDoubleArray = new AtomicDoubleArray(nGroups)
  /**
   * \hat{\beta} sketch: sum of weights of elements in B
   */
